...
theorem SolveLessThanPuzzle {S : Set (Nat → Symbols4)} (H : ∀ f, f ∈ S ↔ LessThanPuzzle f):
  ...
  have c3c7pair: ∀ f (hf:f ∈ S), LockedSet f {3, 7} {1, 2} := by
    intro f hf
    replace H := (H f).mp hf
  ...
  have c0c3pair: ∀ f (hf:f ∈ S), LockedSet f {0, 3} {1, 2} := by
    intro f hf
    replace H := (H f).mp hf
  ...
  replace k := add_fact k 0 1 (by
    intro f hf
    replace H := (H f).mp hf
    replace h := (c0c3pair f hf)
    locked_maps_cases h 0
    · assumption
    · exfalso; exact digit_in_region h H.box1 ((get_d k 5 2) f hf)
  )
